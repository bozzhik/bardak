...
export function registerBotHandlers(bot: Bot): void {
  ...
  bot.command('search', async (ctx) => {
    const identity = getUserIdentity(ctx)
    if (identity !== null && !identity.isBotAccount) {
      logHandledCommand('/search', identity)
    }

    try {
      const result = await handleSearch({identity, query: readStartPayload(ctx.match)}, botDataClient)
      await safelyRecordBotEvent({
        ...getEventActor(identity),
        kind: 'command',
        action: 'search',
      ...
      console.error(`${env.logPrefix} command=/search failed`, error)
      await safelyRecordBotEvent({
        ...getEventActor(identity),
        kind: 'error',
